...
tags:
  ...
  - タコ
private: false
updated_at: '2025-09-01T22:52:56+09:00'
...
:::note info
**`docker-compose.yml` or `compose.yml`どう違う？**（追記）
- 機能としては同じだが、公式は`compose.yml`を推奨しています
- そのため、同じディレクトリに`docker-compose.yml`と`compose.yml`がある場合は、`compose.yml`が優先されるようです

> The default path for a Compose file is compose.yaml (preferred) or compose.yml that is placed in the working directory. Compose also supports docker-compose.yaml and docker-compose.yml for backwards compatibility of earlier versions. If both files exist, Compose prefers the canonical compose.yaml.
<sub>参考：[Docker公式ドキュメント](https://docs.docker.com/compose/intro/compose-application-model/)より</sub>

- 今回は表現を同じにするため、`docker-compose.yml`で統一いたします
:::
